...
void gol_parallel(bool* cells, int n, int m, int iterations, int nthreads) {
    ...
    for(t = 0; t < iterations; ++t) {
        #pragma omp parallel for num_threads(nthreads) private(j,neighbors) shared(cells, cells_new, n,m) schedule(dynamic)
        for(i = 0; i < n; ++i) {
            for(j = 0; j < m; ++j) {
    ...
    if(iterations % 2) {
        #pragma omp parallel for num_threads(nthreads) private(j) shared(cells, cells_new, n, m) schedule(dynamic) 
        for(i = 0; i < n; ++i) {
            for(j = 0; j < m; ++j) {
